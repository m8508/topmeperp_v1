...
-- 可用貸款
    ...
    FROM(
     SELECT B.* , 
	 (SELECT ISNULL(IIF(QUOTA_RECYCLABLE = 'Y', SUM(TRANSACTION_TYPE*AMOUNT),
     ...
     FROM
	 (
	 SELECT PROJECT_ID, SUM(VALUATION_AMOUNT) AS VALUATION_AMOUNT 
     FROM PLAN_VALUATION_FORM GROUP BY PROJECT_ID
	 )va LEFT JOIN
     (SELECT PROJECT_ID, SUM(ITEM_UNIT_PRICE * ITEM_QUANTITY) AS contractAtm 
     FROM PLAN_ITEM GROUP BY PROJECT_ID)pi 
     ON va.PROJECT_ID = pi.PROJECT_ID)V ON B.PROJECT_ID = 
	 V.PROJECT_ID WHERE ISNULL(IS_SUPPLIER, 'N') <> 'Y'
     ) L
)
--輸出結果
  ...
  UNION 
    ...
    SELECT  CONVERT(varchar(10), dbo.getWorkDay(v.EVENT_DATE) , 111) DATE_CASHFLOW,0 AMOUNT_BANK,
	  ISNULL(i.AMOUNT,0) AMOUNT_INFLOW, ISNULL(o.AMOUNT,0) AMOUNT_OUTFLOW,ISNULL(o.AMOUNT_REAL,0) AMOUNT_REAL
	   ...
	   LEFT JOIN Outcome o ON v.EVENT_DATE=o.EVENT_DATE 
	   LEFT JOIN Income i ON v.EVENT_DATE=i.EVENT_DATE
