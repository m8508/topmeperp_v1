...
namespace topmeperp.Service
{
 ...
 SELECT DATE_CASHFLOW,SUM(AMOUNT_BANK) AMOUNT_BANK,SUM(AMOUNT_INFLOW) AMOUNT_INFLOW,Sum(AMOUNT_OUTFLOW) AMOUNT_OUTFLOW,
                ...
                SELECT B.* , (SELECT ISNULL(IIF(QUOTA_RECYCLABLE = 'Y', SUM(TRANSACTION_TYPE*AMOUNT),
                 (SUM(IIF(TRANSACTION_TYPE = 1,0,-1)*AMOUNT))), 0) FROM FIN_LOAN_TRANACTION T WHERE T.BL_ID = B.BL_ID) 
                 SumTransactionAmount,  QUOTA * QUOTA_AVAILABLE_RATIO / 100 AVAILAB
                FROM FIN_BANK_LOAN B LEFT JOIN 
                TND_PROJECT P ON B.PROJECT_ID = P.PROJECT_ID LEFT JOIN 
                ...
                FROM PLAN_ITEM GROUP BY PROJECT_ID)pi 
                ON va.PROJECT_ID = pi.PROJECT_ID)V ON B.PROJECT_ID = V.PROJECT_ID WHERE ISNULL(IS_SUPPLIER, 'N') <> 'Y'
                ) a) LOAN_QUOTA_RUNNING_TOTAL                
                ,SUM(SUM(AMOUNT_BANK)+ SUM(AMOUNT_INFLOW)-Sum(AMOUNT_OUTFLOW)) OVER(ORDER BY DATE_CASHFLOW
                     ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) 
                          AS CASH_RUNNING_TOTAL
                FROM (
                --加入銀行存款
                SELECT CONVERT(varchar(10), GETDATE() , 111) DATE_CASHFLOW,SUM(CUR_AMOUNT) AMOUNT_BANK,0 AMOUNT_INFLOW,0 AMOUNT_OUTFLOW,0.AMOUNT_REAL FROM FIN_BANK_ACCOUNT
                UNION 
                SELECT  CONVERT(varchar(10), dbo.getWorkDay(v.EVENT_DATE) , 111) DATE_CASHFLOW,0 AMOUNT_BANK,
                 ISNULL(INAMOUNT,0) AMOUNT_INFLOW,ISNULL(OUTCOME.AMOUNT,0) AMOUNT_OUTFLOW,OUTCOME.AMOUNT_REAL
                  from vw_TimeIndex4CashFlow v 
                LEFT JOIN (
                SELECT CONVERT(varchar(10),dbo.getWorkDay(dbo.getWorkDay(EVENT_DATE)) , 111) EVENT_DATE,SUM(AMOUNT) AMOUNT,SUM(AMOUNT_REAL) AMOUNT_REAL FROM (
...
ON R.SUBJECT_ID=S.FIN_SUBJECT_ID WHERE S.BUDGET_DAY IS NULL 
                ...
                LEFT JOIN FIN_EXPENSE_ITEM i ON f.EXP_FORM_ID = i.EXP_FORM_ID WHERE i.FIN_SUBJECT_ID Like '6%'
                AND f.STATUS = 30 GROUP BY PAYMENT_DATE,FIN_SUBJECT_ID) REALAMT
...
WHERE S.BUDGET_DAY IS NULL 
                ...
                AND f.STATUS = 30 GROUP BY PAYMENT_DATE,FIN_SUBJECT_ID) REALAMT
                ON BUDGET.EVENT_DATE=REALAMT.EVENT_DATE AND BUDGET.SUBJECT_ID=REALAMT.SUBJECT_ID ) EX
                GROUP BY EVENT_DATE 
                UNION --其他帳務借款
                ...
                GROUP BY ISNULL(EVENT_DATE,PAYBACK_DATE) 
                ) O GROUP BY dbo.getWorkDay(EVENT_DATE)
                ) OUTCOME
                ON dbo.getWorkDay(v.EVENT_DATE)=dbo.getWorkDay(OUTCOME.EVENT_DATE)
                LEFT JOIN  --收入
                (--廠商還款
                SELECT dbo.getWorkDay(EVENT_DATE) EVENT_DATE ,SUM(AMOUNT) INAMOUNT FROM (
                ...
                GROUP BY dbo.getWorkDay(EVENT_DATE)
                ) INCOME
                ON dbo.getWorkDay(v.EVENT_DATE)=dbo.getWorkDay(INCOME.EVENT_DATE) 
                WHERE AMOUNT <>0 OR INAMOUNT <>0  or AMOUNT_REAL is not null 
                 ) Ve
                GROUP BY DATE_CASHFLOW ORDER BY DATE_CASHFLOW";
                logger.Debug("SQL=" + sql);
