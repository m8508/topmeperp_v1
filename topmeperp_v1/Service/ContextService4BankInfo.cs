    ...
    public class ContextService4BankInfo : ContextService
    {
        ...
        public List<BankLoanInfoExt> getAllBankLoan()
        {
            ...
            using (var context = new topmepEntities())
            {
                try
                {
                    string sql = "SELECT B.* , P.PROJECT_NAME, ISNULL(ROUND(V.vaRatio * 100, 0),0) AS vaRatio  , (SELECT ISNULL(IIF(QUOTA_RECYCLABLE = 'Y', SUM(TRANSACTION_TYPE*AMOUNT),(SUM(IIF(TRANSACTION_TYPE = 1,0,-1)*AMOUNT))), 0) FROM FIN_LOAN_TRANACTION T WHERE T.BL_ID = B.BL_ID) SumTransactionAmount " +
                                 "FROM FIN_BANK_LOAN B LEFT JOIN TND_PROJECT P ON B.PROJECT_ID = P.PROJECT_ID LEFT JOIN (SELECT va.PROJECT_ID AS PROJECT_ID, ISNULL(va.VALUATION_AMOUNT, 0) / ISNULL(pi.contractAtm, 1) AS vaRatio FROM(SELECT PROJECT_ID, SUM(VALUATION_AMOUNT) AS VALUATION_AMOUNT " +
